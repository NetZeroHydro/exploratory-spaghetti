...
box_poly <- st_geometry(nepal_basin)

box_poly <- st_transform(box_poly, st_crs(nepal_current_dams))

# Now filter (returns sf)
trial_current_dams <- nepal_current_dams %>%
  st_filter(y = box_poly, .predicate = st_intersects)

class(trial_current_dams)
nrow(trial_current_dams)
  ...
  # Get coordinates of current dams (use same approach as future dams)
  coords_current <- st_coordinates(trial_current_dams_proj)
  
  # Extract X and Y - try column names first (like future dams), then fall back to position
  if ("X" %in% colnames(coords_current)) {
    x_current <- as.numeric(coords_current[, 'X'])
    y_current <- as.numeric(coords_current[, 'Y'])
  } else {
    # Fall back to column position
    x_current <- as.numeric(coords_current[, 1])
    y_current <- as.numeric(coords_current[, 2])
  }

  # Ensure we have valid coordinates
  cat("Current dams coordinates - X:", length(x_current), "Y:", length(y_current), "\n")
  cat("X range:", range(x_current), "Y range:", range(y_current), "\n")

  # Snap current dams to river network (use exact same approach as future dams)
  current_dams_network <- xy2segvert(x = x_current, 
                                      y = y_current, 
                                      rivers = trial_river_network)
  n_current <- length(current_dams_network$vert)
}
...
# Handle case where there are no current dams
if (n_current > 0 && !is.null(current_dams_network)) {
  all_dams_vert <- c(current_dams_network$vert, xy_line$vert)
  all_dams_seg <- c(current_dams_network$seg, xy_line$seg)
} else {
  # Only future dams
  all_dams_vert <- xy_line$vert
  all_dams_seg <- xy_line$seg
  cat("Note: Using only future dams (no current dams in trial area)\n")
}
...
if (n_current > 0) {
  ...
  dam_labels <- paste0("Future_", 1:n_future)
}
