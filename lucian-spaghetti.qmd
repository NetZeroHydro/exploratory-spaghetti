...
Make all geometries valid to ensure spatial operations work correctly.
...
Clip all spatial data to the Nepal basin boundary using spatial intersection.
...
```{r}
# First, get all rivers in Nepal basin
...
# River Order Visualization

Visualize all river orders (1-9) with color coding to show network hierarchy.

```{r}
# Filter rivers to order 1 and 2 for complete visualization
...
# Map all river orders with color coding
#tmap_mode("view")
...
# Create graph object
nepal_graph = igraph::tbl_graph(nodes = nepal_nodes, edges = as_tibble(nepal_edges), directed = FALSE)
```
...
nepal_rivers_3up_snap <- st_set_geometry(nepal_rivers_3up, "geometry")
nepal_rivers_3up_snap <- st_transform(nepal_rivers_3up_snap, crs = 3857)
nepal_current_dams_snap <- st_transform(nepal_current_dams, crs = 3857)
