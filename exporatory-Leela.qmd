...
nepal_rivers_vect <- vect(nepal_rivers)
crs(pop) # no crs, we will assign 4326
pop <- project(pop, "epsg:4326")
crs(pop) # looks good

res(pop) == res(nepal_rivers_vect)

pop_nepal <- crop(pop, nepal_rivers_vect)

net_with_dams_vect <- vect(net_with_dams_df)
...
```{r}
# plot populations and nepal rivers
plot(pop_nepal_nozero) 
#plot(net_with_dams_df, add = TRUE, col = "orange")
plot(nepal_rivers_vect, add = TRUE, col = "orange", alpha = 0.3)
```
```{r}
# make future dams a terra object
...
nepal_future_buffer <- buffer(nepal_future_dams_vect, width = 1000)
